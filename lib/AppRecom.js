  ...
  _createClass(AppRecom, [{
    ...
    value: function _getRules(ogData, itemsets, min_conf) {
      ...
      [].concat(_toConsumableArray(itemsets.keys())).forEach(function (itemset) {
        ...
        if (hypFreq / conFreq >= min_conf) _this3._addRules(rules, hyp, con, count); // could do this recursively,
        else if (conFreq / hypFreq >= min_conf) _this3._addRules(rules, con, hyp, count); // but in our case our data will only have 2 attributes
      });
